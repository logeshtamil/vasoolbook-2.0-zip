...
Set the `GOOGLE_WEB_CLIENT_ID` environment variable **before** running any `cap`/Gradle
command. Both `capacitor.config.js` and `android/app/build.gradle` read the same variable
name, so you only set it once. Pick one:
...
- **Persistent, per-developer (recommended):** add a line to `android/local.properties`
  ...
  ```
  This is read automatically by `android/app/build.gradle`. Note: `capacitor.config.js`
  only reads the environment variable (Node has no access to `local.properties`), so if
  you rely on `local.properties`, also export the env var before running `npx cap sync`.
...
### 1.3 Install the real plugin bundle
`www/js/vendor/capacitor-google-auth.js` currently contains a placeholder (it
intentionally throws/warns instead of pretending to work). Replace it with the real
build:
```bash
npm install
npx cap sync android
cp node_modules/@codetrix-studio/capacitor-google-auth/dist/plugin.js \
   www/js/vendor/capacitor-google-auth.js
```
